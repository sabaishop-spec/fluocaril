...
import { PanelBottom, Save, CheckCircle, Download } from 'lucide-react';
...
export function FooterForm({ initialData }: { initialData: FooterData | null }) {
  const defaults = {
    ...
    if (typeof window !== 'undefined') {
      ...
      if (saved) {
        try {
          ...
          alert('Đã nhập dữ liệu cũ từ trình duyệt. Hãy kiểm tra lại trước khi Lưu.');
        } catch (e) {
  ...
  const handleSave = async () => {
    setIsSaving(true);
    const result = await saveSetting('footer_settings', data);
    setIsSaving(false);
    if (result.success) {
      setShowToast(true);
    ...
    <div className="p-8 max-w-7xl mx-auto relative">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Link Facebook (Để trống sẽ ẩn)</label>
            <input 
              type="url" 
              value={data.facebook}
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Link Instagram (Để trống sẽ ẩn)</label>
            <input 
              type="url" 
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Link TikTok (Để trống sẽ ẩn)</label>
            <input 
              type="url" 
              value={data.tiktok}
          ...
          <div className="md:col-span-2 pt-4 border-t border-gray-100 mt-2">
            ...
            <input 
              ...
              className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-teal-500 outline-none transition-shadow bg-white text-sm text-gray-500"
            />
          </div>
        </div>
      </div>
